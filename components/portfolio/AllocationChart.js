...
function CustomLegend({ payload, allocation }) {
  return (
...
export default function AllocationChart({ allocation }) {
  ...
  return (
    <div className="w-full">
          ...
          >
            {data.map((entry) => (
              <Cell
                ...
                fill={ASSET_CLASS_COLORS[entry.assetClass] ?? '#94a3b8'}
              />
            ))}
          </Pie>
          <Tooltip content={<CustomTooltip />} />
          <Legend
            content={(props) => <CustomLegend {...props} allocation={allocation} />}
            layout="vertical"
            align="right"
      ...
      {/* Target vs Current bar comparison */}
      <div className="mt-4 space-y-2">
        {allocation.map((item) => {
          const drift = item.currentPct - item.targetPct;
          return (
              ...
              <div className="relative h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                ...
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: ASSET_CLASS_COLORS[item.assetClass] ?? '#94a3b8',
                    opacity: 0.85,
                  }}
                />
              </div>
