...
function CustomTooltip({ active, payload, label }) {
  ...
  const portfolio = payload.find((p) => p.dataKey === 'portfolioReturn');
  const benchmark = payload.find((p) => p.dataKey === 'benchmarkReturn');
      ...
      {payload[0]?.payload?.portfolioValue && (
        <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700">
          <span className="text-slate-500 dark:text-slate-400">
            Portfolio value: <span className="font-medium text-slate-900 dark:text-slate-100">{formatCurrency(payload[0].payload.portfolioValue)}</span>
          </span>
        </div>
      )}
    </div>
