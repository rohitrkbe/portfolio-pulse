...
import { RISK_PROFILE_LIST, SORT_FIELDS, SORT_ORDERS } from '@/lib/constants';
import { SearchIcon, ArrowUpDownIcon } from '@/components/ui/Icons';
import { DASHBOARD_STRINGS } from '@/lib/strings';
...
export default function FilterSortBar({ totalVisible, totalAll }) {
  ...
  const handleSearch = useCallback(
    (value) => {
      setInputValue(value); // immediate — keeps input cursor in sync
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
        ...
        {/* Search input — local state + debounced transition dispatch */}
            ...
            <button
              ...
              onClick={() =>
                sort.field === opt.field
                  ...
                  : dispatch(setSort({ field: opt.field, order: SORT_ORDERS.DESC }))
              }
